...
export const eventService = {
  ...
  async join(id: number, token: string) {
    const res = await axios.post(`${API_BASE}/events/${id}/join`, {}, {
      headers: { Authorization: `Bearer ${token}` },
    });
    return res.data;
  },
  ...
  async leave(id: number, token: string) {
    const res = await axios.post(`${API_BASE}/events/${id}/leave`, {}, {
      headers: { Authorization: `Bearer ${token}` },
    });
    return res.data;
  },
  ...
  async attendees(id: number, params?: { skip?: number; limit?: number }) {
    const res = await axios.get(`${API_BASE}/events/${id}/attendees`, { params });
    return res.data;
  },
};
